...
def main(user_response): #ini program utama yang menghandle chat dari user
    if(user_response!='bye'):
        if(user_response=='thanks' or user_response=='thank you' ):
            global flag
            flag=False
            print("RUKA: Sama sama..")
        else:
            if len(user_response)==0:
                print('RUKA:  {}'.format(random.choice(Confused_responses)))
            elif(greeting(user_response)!=None):
                print("RUKA: "+greeting(user_response))
            else:
                print("RUKA:", ''+response(user_response))
...
print("RUKA: Namaku Ruka. Aku bakal ngejawab pertanyaan tentang Fasilkom UI, tempat dimana pembuatku berkuliah yang ada di database ku.")
print('RUKA: Bot ini memiliki beberapa fitur\n\
      Fitur pengingat, "ingetin saya untuk (aktifitasmu) dalam (waktu) (menit atau jam)"\n\
      Kalau mau keluar, ketik bye!')
print('      Pesan pembuat: Rukabot ini tidak sempurna, ada kemungkinan jawaban dari bot tidak nyambung karena bot ini menggunakan metode cosine similarity.')
...
while(flag==True):
    ...
    if 'ingetin saya untuk' in user_response:
        ...
        try:    
            ...
            print("RUKA: kau tahu, kau gabisa input angka desimal karena aku bilang begitu.")
            print("RUKA: input bilangan bulat, bukan desimal 'manoosia cerdas'.")
            continue
        acara=perintah[3:-3]
        angka=perintah[-2]
